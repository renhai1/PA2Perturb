...
# 将被选中的代表性节点加入训练集
def node_sample_and_save_my(data, k, folder, num_classes, idx_attach):
    # 获取标签
    labels = data.y.to('cpu')
    ...
    # 将 idx_attach 中的节点索引加入训练集
    attach_idx_tensor = idx_attach.clone()
    attach_labels = labels[idx_attach]

    # 统计 idx_attach 中每个类别的节点数量
    class_counts = {i: 0 for i in range(num_classes)}
    for label in attach_labels:
        class_counts[label.item()] += 1

    # 剩余的节点索引
    remaining_idx = torch.arange(data.num_nodes)
    mask = torch.ones(data.num_nodes, dtype=torch.bool)
    mask[idx_attach] = False
    remaining_idx = remaining_idx[mask]
    remaining_labels = labels[remaining_idx]

    # 从剩下的数据中选出所需数量的样本作为训练集（排除已选的代表性节点）
    for i in range(num_classes):
        class_indices = remaining_idx[remaining_labels == i]
        if class_counts[i] < k:
            num_to_select = k - class_counts[i]
            selected_indices = class_indices[:num_to_select]
            train_idx.append(attach_idx_tensor[attach_labels == i])
            train_idx.append(selected_indices)
            train_labels.append(attach_labels[attach_labels == i])
            train_labels.append(labels[selected_indices])
    ...
    # 剩下的节点作为测试集
    test_idx = remaining_idx[~torch.isin(remaining_idx, train_idx)]
    test_labels = labels[test_idx]

    # 确保测试集和训练集没有重叠
    assert not torch.any(torch.isin(test_idx, train_idx)), "Test set and training set overlap!"
...
def load4graph(dataset_name, shot_num=10, num_parts=None, pretrained=False):
    ...
    if dataset_name in GRAPH_TASKS:
        dataset = TUDataset(root='data/TUDataset', name=dataset_name,
                            use_node_attr=True)  # use_node_attr=False时，节点属性为one-hot编码的节点类别
